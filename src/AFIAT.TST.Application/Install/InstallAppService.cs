...
using AFIAT.TST.Migrations.Seed.Host;
...
namespace AFIAT.TST.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : TSTAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<TSTDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{TSTConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{TSTConsts.ConnectionStringName}", constring);
        }
